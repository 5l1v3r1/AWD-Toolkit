...
//全局输入检测  基本的url和post检测过了则对所有输入进行简单过滤
    public function gloabel_attack_detect()
    {
...
//记录每次大概访问记录，类似日志，以便在详细记录中查找
    public function write_access_log_probably()
    {
...
//记录详细的访问头记录，包括GET POST http头   以获取通防waf未检测到的攻击payload
    public function write_access_logs_detailed()
    {
 ...
 */
    public function filter_attack_keyword($str)
    {
...
简单将易出现问题的字符替换成中文
 */
    public function filter_dangerous_words($str)
    {
        $str = str_replace("'", "‘", $str);
        $str = str_replace("\"", "“", $str);
        $str = str_replace("<", "《", $str);
 ...
 */
    public function write_attack_log($alert)
    {
        $data = date("Y/m/d H:i:s") . " -- [" . $alert . "]" . "\r\n" . $this->get_http_raws() . "\r\n\r\n";
        $ffff = fopen('/tmp/attack_detected_log.txt', 'a'); //日志路径
        fwrite($ffff, $data);
        fclose($ffff);
        if ($alert == 'GETFLAG') {
            echo "flag{Thequickbrownfoxjumpsoveralazydog}"; //如果请求带有flag关键字，显示假的flag。（2333333）
        } else {
            sleep(3); //拦截前延时3秒
        }
        exit(0);
